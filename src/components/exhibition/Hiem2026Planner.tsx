...
function MultiPhotoUploadField({ photos, onChange }: { photos: string[]; onChange: (photos: string[]) => void }) {
  ...
  return <label className="block text-sm font-semibold">구매요청 샘플 사진 업로드<span className="mt-1.5 flex h-10 cursor-pointer items-center gap-2 border border-dashed border-[#999] px-3 text-xs font-normal text-[#555]"><ImagePlus className="h-4 w-4" />{photos.length ? `${photos.length}장 첨부 · 추가 선택` : '여러 이미지 선택'}<input type="file" accept="image/*" multiple className="sr-only" onChange={(event) => addPhotos(event.target.files)} /></span>{photos.length > 0 && <span className="mt-2 grid grid-cols-3 gap-2">{photos.map((photo, index) => <img key={`${photo.slice(0, 32)}-${index}`} src={photo} alt={`구매요청 샘플 사진 ${index + 1}`} className="h-20 w-full border border-[#ccc] object-cover" />)}</span>}</label>
}
