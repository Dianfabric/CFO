...
export default function Hiem2026Planner() {
    ...
    const load = async () => {
      try {
        ...
        records.forEach((record: any) => {
          ...
          if (record.isCustom) nextCustom.push({ id: record.boothId, brand: record.brand, hall: record.hall, booth: record.boothCode, status: 'confirmed', photo: record.boothPhotoUrl ?? undefined, businessCard: record.businessCardUrl ?? undefined, createdAt: record.createdAt })
  ...
  const addCustomBooth = async (booth: CustomBooth) => {
    try {
      ...
      const body = await response.json(); if (!response.ok) throw new Error(body.error ?? '저장 실패')
      setCustomBooths((current) => [...current, { ...booth, photo: body.record.boothPhotoUrl ?? undefined, businessCard: body.record.businessCardUrl ?? undefined }]); setNotes((current) => ({ ...current, [booth.id]: applyRecord(body.record) })); setCreateOpen(false); setSelectedId(booth.id); setSyncMessage('Supabase 공유 저장')
...
function MultiPhotoUploadField({ photos, photoUrls, onChange }: { photos: string[]; photoUrls: Record<string, string>; onChange: (photos: string[]) => void }) {
  ...
  const addPhotos = async (files: FileList | null) => {
    ...
    const added = await Promise.all(imageFiles.map((file) => new Promise<string>((resolve) => { const reader = new FileReader(); reader.onload = () => resolve(String(reader.result ?? '')); reader.readAsDataURL(file) })))
...
function CustomBoothAttachments({ booth }: { booth: CustomBooth }) {
  const attachments = [{ label: '업체 사진', url: booth.photo }, { label: '명함', url: booth.businessCard }].filter((item): item is { label: string; url: string } => Boolean(item.url))
  if (!attachments.length) return null
  return <div className="mt-4 grid grid-cols-2 gap-2">{attachments.map((item) => <a key={item.label} href={item.url} target="_blank" rel="noreferrer" className="block overflow-hidden border border-[#ccc] bg-[#f7f7f7] hover:border-[#76b900]"><img src={item.url} alt={`${item.label} 크게 보기`} className="h-20 w-full object-cover" /><span className="block px-2 py-1.5 text-xs font-semibold">{item.label} · 크게 보기</span></a>)}</div>
}
