...
assert.equal(hiem2026Booths.filter((booth) => booth.status === 'confirmed').length, 27)
...
assert.equal(hiem2026Booths.find((booth) => booth.brand === 'DT')?.booth, 'F48')
assert.equal(hiem2026Unavailable.includes('NOVATEX'), true)
assert.deepEqual(boothSummary(hiem2026Booths), { confirmed: 27, pending: 1 })
