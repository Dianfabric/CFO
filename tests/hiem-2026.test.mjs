...
assert.equal(hiem2026Booths.filter((booth) => booth.status === 'confirmed').length, 25)
assert.equal(hiem2026Booths.filter((booth) => booth.status === 'pending').length, 3)
...
assert.deepEqual(boothSummary(hiem2026Booths), { confirmed: 25, pending: 3 })
