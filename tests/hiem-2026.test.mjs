...
assert.equal(hiem2026Booths.filter((booth) => booth.status === 'confirmed').length, 24)
assert.equal(hiem2026Booths.filter((booth) => booth.status === 'pending').length, 4)
...
assert.deepEqual(boothSummary(hiem2026Booths), { confirmed: 24, pending: 4 })
